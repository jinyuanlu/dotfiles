...
```bash
NEW_TITLE=$(~/.claude/skills/bin/pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
...
**Persist the result:**
```bash
~/.claude/skills/bin/review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```

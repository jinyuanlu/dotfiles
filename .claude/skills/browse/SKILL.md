...
~/.claude/skills/bin/timeline-log '{"skill":"browse","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
`$B` is the browse binary (resolved from `$_ROOT/.claude/skills/browse/dist/browse` or `~/.claude/skills/browse/dist/browse`).

...
```bash
source <(~/.claude/skills/bin/slug 2>/dev/null) && mkdir -p ~/.gstack/projects/$SLUG/ceo-plans
```
...
```bash
~/.claude/skills/bin/review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```

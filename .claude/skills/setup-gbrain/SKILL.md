...
~/.claude/skills/bin/timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/bin/question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/bin/gbrain-detect
```
...
**If A (Retry)**: re-run `~/.claude/skills/bin/gbrain-detect`
...
```bash
~/.claude/skills/bin/gbrain-install
```
...
```bash
. ~/.claude/skills/bin/gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.claude/skills/bin/gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.claude/skills/bin/gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.claude/skills/bin/gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.claude/skills/bin/gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.claude/skills/bin/gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.claude/skills/bin/gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.claude/skills/bin/gbrain-install || exit $?
...
```bash
current_tier=$(~/.claude/skills/bin/gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.claude/skills/bin/gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.claude/skills/bin/artifacts-init --url-form-supported "$URL_FORM"
~/.claude/skills/bin/config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.claude/skills/bin/gbrain-source-wireup --strict \
...
```bash
~/.claude/skills/bin/memory-ingest --probe
```
...
After answer:
```bash
~/.claude/skills/bin/config set transcript_ingest_mode <choice>
~/.claude/skills/bin/gbrain-sync --full --no-brain-sync
```
...
```bash
_HASH=$(~/.claude/skills/bin/config endpoint-hash 2>/dev/null)
_POLICY=$(~/.claude/skills/bin/config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.claude/skills/bin/config set brain_trust_policy@$_HASH personal
...
```bash
~/.claude/skills/bin/config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.claude/skills/bin/config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.claude/skills/bin/config set artifacts_sync_mode full
...
```bash
~/.claude/skills/bin/gbrain-detect 2>/dev/null || true
~/.claude/skills/bin/config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.claude/skills/bin/config get artifacts_sync_mode 2>/dev/null || echo "off"

...
```bash
source <(~/.claude/skills/bin/diff-scope <base> 2>/dev/null) || true
...
```bash
~/.claude/skills/bin/specialist-stats 2>/dev/null || true
```
...
```bash
~/.claude/skills/bin/learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
```bash
~/.claude/skills/bin/review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
OLD_CFG=$(~/.claude/skills/bin/config get codex_reviews 2>/dev/null || true)
...
```bash
~/.claude/skills/bin/review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/bin/learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
2. Compute the corrected title: `NEW_TITLE=$(~/.claude/skills/bin/pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).

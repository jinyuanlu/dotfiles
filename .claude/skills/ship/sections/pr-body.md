...
<PR body from above>
PR_BODY_EOF
~/.claude/skills/bin/redact --from-file "$PR_BODY_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json
case $? in
...
printf '%s' "v$NEW_VERSION <type>: <summary>" | ~/.claude/skills/bin/redact --repo-visibility "$REDACT_VIS" --json
```
